...
googleMaps = {
		...
		createMarker: function(idPOI,name,lat,lng,html,POIURL) {
			    ...
			    google.maps.event.addListener(marker, 'click', function() {
			        ...
			        googleMaps.displayPanorama(POIURL);
			    });
		   ...
		   getWS: function(url, success, fail){
			...
			displayPanorama: function(POIURL){
				$("#viewer360").empty();
				if (!$("#viewer360").is(":visible")) {
					$("#viewer360").animate({width:'toggle'});
				}
				$("#viewer360").append('<iframe src="https://googledrive.com/host/0Bxv48jdysarPb09ZUEo2WmJlTXM/gilda_in.html"></iframe>');	
			},
			linkListMarker: function(){
				$.each(googleMaps.markerArray, function(index,value){
					...
					$("."+value.id).click(function() {
						    ...
						    googleMaps.displayPanorama(value.POIURL);
					});
				})
			},
			poiWindow: function(){
			...
			googleMaps.createMarker(unique_id,value["POI_name"],latitude,longitude,"<span>"+value["POI_description"]+"</span>",POIURL);
